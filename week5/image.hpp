...
// using this for BMP / NIfTI file I/O
#include "bmpmini.hpp"
#include "niftiio.hpp"
  ...
  public:
    ...
    const imageNd<value_type> getSlice( size_t dim, size_t sli, std::string filename="" ) {
        ...
        if ( !filename.empty() ) {
			...
			auto
			  minvalue = *std::min_element ( out.data.begin(), out.data.end() ),
			  maxvalue = *std::max_element ( out.data.begin(), out.data.end() );
			float
			  scalefactor = ( maxvalue - minvalue ) / 256.;		
			
			// rescale image intensities to bitmap range [ 0..255 ]
			std::transform ( out.data.begin(), out.data.end(), bmpdata, 
    ...
    bool GetWatershedImage() {
        ...
        for (size_t k=1; k < max_value+1; k++)
        ...
        for (size_t k=0; k< max_value+1; k++)
