...
namespace aip {
    ...
    class image2d {
	  ...
	  uint8_t* 
	    bmpdata  = new uint8_t [ width * height ];
	  auto
	    minvalue = *std::min_element ( data.begin(), data.end() ),
	    maxvalue = *std::max_element ( data.begin(), data.end() ),
	    scalefactor = ( maxvalue - minvalue ) / 256;		
	  image::ImageView 
	    bmpimageview ( (int) width, (int) height, 1, bmpdata );			
	  image::BMPMini 
	    bmpimage;
      
	  // rescale image intensities to bitmap range [ 0..255 ]
	  std::transform ( data.begin(), data.end(), bmpdata, 
			   [&]( uint8_t c ) { return ( ( c - minvalue ) / scalefactor ); } );
	  
	  // write bitmap, throw exception if unsuccessful
	  try { 
	    bmpimage.write ( bmpimageview, filename ); 
	  } catch (const std::exception& e) { 
	    std::cout << e.what() << std::endl; 
	  }
      
	  // accompany every new with delete!
	  delete[] bmpdata;
    ...
    class imageNd {
        ...
        const imageNd<T>& operator+= ( const value_type& rhs ) {
			// 1A design the += operator for adding a value to an image 
             for (size_t s = 0; s < data.size(); s++ )
                data[s] += rhs;
            return (*this);
        }
        ...
        const imageNd<T>& operator+= ( const imageNd<U>& rhs ) {
            for (size_t s = 0; s < data.size(); s++ )
                data[s] += rhs.data[s];
            return (*this);
			// 1 B design the += operator for adding an image to the image
			// 1 C why are there 2 += operators, and what is this called?
        }
        ...
        const imageNd<T>& operator+ ( const U& rhs ) {
            ...
            out += rhs;
            return out;
			// 1 D design the + operator for an image
			// 1 E why is there only 1 +operator and not 2 like with += ?
        }
        ...
        std::vector<value_type> getSlice( size_t dim, size_t sli ) {
            ...
            if (sizes.size() != 3) {
                ...
                case (0):
                    ...
                    dptr = &data [ sli * strides[0] ]; // pointer in imageNd
                ...
                case (2):
                    ...
                    for (size_t c=0; c<ovec.size(); c++, optr++, dptr++)
                        *optr = *dptr;
                        // 2 A insert the code that copies a z-slice
						//     (you can do this with pointers or brackets!)
						// 2 B why is this for-loop so much 'easier' to write?
		...
		// initialise all the necessary variables
		uint8_t* 
		  bmpdata  = new uint8_t [ slicesize ];
		auto
		  minvalue = *std::min_element ( slice.begin(), slice.end() ),
		  maxvalue = *std::max_element ( slice.begin(), slice.end() ),
		  scalefactor = ( maxvalue - minvalue ) / 256;		
		
		// rescale image intensities to bitmap range [ 0..255 ]
		std::transform ( slice.begin(), slice.end(), bmpdata, 
				 [&]( uint8_t c ) { return ( ( c - minvalue ) / scalefactor ); } );
	    
		// Vertical flip
		for (size_t i = 0; i < xsize; i++) {
			for (size_t j = 0; j < ysize / 2; j++) {
				std::swap(bmpdata[j * xsize + i], bmpdata[(ysize - j - 1) * xsize + i]);
			}
		}
		...
		for (size_t j = 0; j < ysize; j++) {
			for (size_t i = 0; i < xsize / 2; i++) {
				std::swap(bmpdata[j * xsize + i], bmpdata[j * xsize + (xsize - i - 1)]);
			}
		}

		// write bitmap, throw exception if unsuccessful 
		image::ImageView 
		  bmpimageview ( xsize, ysize, 1, bmpdata );
		image::BMPMini 
		  bmpimage;
		try { 
		  bmpimage.write ( bmpimageview, filename ); 
