...
int main() {
    ...
    // the constructor that initialises the pixels, is called
    ...
    nifti_image
        *inputimage = nifti_image_read ( "../data/brainT1.nii.gz",
                                         DO_READ_DATA );
    if (!inputimage) {
        std::cerr << "Error: Unable to read NIfTI image." << std::endl;
        return -1;
    }
    ...
    for (int i=0; i < ( 256*256 ); i++ )
        my_floatimage.setpixel( i,float ( pixeldata[ slice_offset += 182 ] ) );
